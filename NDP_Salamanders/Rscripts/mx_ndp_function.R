maxent_ndp <- function(mx_sp1, mx_sp2, cut_off = 0.95) {
    ...
    A <- mx_sp1$response.plots[[1]]$data |> dplyr::filter(source == "Suitability") #|> dplyr::filter(grepl(taxa_comb[j, 1], category))
    B <- mx_sp2$response.plots[[1]]$data |> dplyr::filter(source == "Suitability") #|> dplyr::filter(grepl(taxa_comb[j, 2], category))
    ...
    C <- 
        ...
        dplyr::arrange(layer) |> 
        dplyr::mutate(value.x = na.approx(value.x, x = layer, na.rm = F)) |>
        dplyr::mutate(value.y = na.approx(value.y, x = layer, na.rm = F)) |> 
        dplyr::select(1, 2, 4) |>
    ...
    # Calculate area
    NicheDiss <- 1 - (pracma::trapz(C[[1]], C[[2]])/pracma::trapz(A[[1]], A[[2]]) + pracma::trapz(C[[1]], C[[2]])/pracma::trapz(B[[1]], B[[2]]))/2
