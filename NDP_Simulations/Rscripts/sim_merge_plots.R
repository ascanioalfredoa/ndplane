...
sim_files <- list.files("Results/sim_output_v2/", full.names = TRUE)


#### Merging all tables into 1 ####
sim <- read.csv(sim_files[1])
write.table(sim, "Results/full_sim_output_v2.csv", row.names = F, sep = ",")

for(i in 2:length(sim_files)) {
    sim <- read.csv(sim_files[i])
    write.table(sim, "Results/full_sim_output_v2.csv", append = T, row.names = F, sep = ",", col.names = F)
    if(i %% 100 == 0) print(i)
}

#### load full data ####
simdata <- read_csv("Results/full_sim_output_v2.csv")
Sys.Date()
...
png("Figures/sim_plot_nested1.png", width = 2000, height = 1000, res = 300)
nested_case1 %>%
...
png("Figures/sim_plot_weighted1.png", width = 2000, height = 1000, res = 300)
weighted_case1 %>%
    ggplot(aes(x = Exclusivity, y = Dissimilarity), col = "red") +
    geom_point() +
    xlim(0, 1) + ylim(0, 1) +
...
png("Figures/sim_plot_csh1.png", width = 2000, height = 1000, res = 300)
csh_case1 %>%
    ggplot(aes(x = Exclusivity, y = Dissimilarity), col = "red") +
    geom_point() +
    xlim(0, 1) + ylim(0, 1) +
...
png("Figures/sim_plot_nested2.png", width = 2000, height = 1000, res = 300)
nested_case2 %>%
    ggplot(aes(x = Exclusivity, y = Dissimilarity), col = "red") +
    geom_point() +
    xlim(0, 1) + ylim(0, 1) +
...
png("Figures/sim_plot_weighted2.png", width = 2000, height = 1000, res = 300)
weighted_case2 %>%
    ggplot(aes(x = Exclusivity, y = Dissimilarity), col = "red") +
    geom_point() +
    xlim(0, 1) + ylim(0, 1) +
