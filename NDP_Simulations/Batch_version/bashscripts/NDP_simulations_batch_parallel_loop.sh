...
#Comment SBATCH --ntasks-per-node=24
#SBATCH --ntasks=1
...
#do
    ...
    R CMD BATCH --no-restore --no-save rscripts/Simulate_NDP_Space_Parallel.R slurm_out/output$SLURM_ARRAY_TASK_ID
...
ls /tmp -l | grep "Rtmp" | grep "ascaniaa" | cut -c 61-70 | xargs rm -rf
