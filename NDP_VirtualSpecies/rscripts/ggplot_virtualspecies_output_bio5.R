...
vsp_files <- list.files("output/virtualspecies_bio5_sim_output/", full.names = T)

vsp_sims <- read_csv(vsp_files)

#### Check range of D and I ####
vsp_sims$D[vsp_sims$D < 0] <- 0

range(vsp_sims$D)
range(vsp_sims$I)

unique(round(vsp_sims$D, 2))
table(round(vsp_sims$D, 2))

unique(round(vsp_sims$I, 2))
table(round(vsp_sims$I, 2))
...
vsp_sims_D <- vsp_sims %>%
    filter(round(D, 2) %in% round(seq(0.15, 1, 0.25), 2))
...
gg_a <- 
    vsp_sims_D %>%
    ggplot(aes(x = Exclusivity, y = Dissimilarity, color = as.factor(round(D, 2)))) +
...
vsp_sims_I <- vsp_sims %>%
    filter(round(I, 2) %in% round(seq(0.40, 0.95, 0.15), 2))

png(filename = "Figures/vsp_I_NDP.png", width = 5000, height = 4500, res = 600)
gg_b <- 
    vsp_sims_I %>%
    ggplot(aes(x = Exclusivity, y = Dissimilarity, color = as.factor(round(I, 2)))) +
    geom_point(alpha = 0.5) +
...
png(filename = "Figures/Fig7_vsp.png", width = 11000, height = 9000, res = 600)
p
